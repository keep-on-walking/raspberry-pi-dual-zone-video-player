...
class MPVInstance:
    ...
    def _build_command(self, source):
        """Build MPV command with all necessary flags for headless DRM output"""
        
        # Calculate position and scaling for DRM/KMS
        # MPV in DRM mode uses different positioning approach
        cmd = [
            'mpv',
            ...
            # DRM/KMS video output (headless, no X11)
            '--vo=gpu',
            '--gpu-context=drm',
            '--drm-connector=HDMI-A-1',  # Primary HDMI output
            ...
            # Window forcing (ensures output even with no video)
            '--force-window=yes',
            '--idle=yes',
            ...
            # Video scaling and positioning via lavfi
            # This is where we control position and size in DRM mode
            f'--vf=lavfi=[scale={self.geometry["width"]}:{self.geometry["height"]},pad={self.geometry["width"]}:{self.geometry["height"]}:{self.geometry["x"]}:{self.geometry["y"]}]',
